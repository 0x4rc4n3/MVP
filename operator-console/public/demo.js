...
function initTabDemo() {
  ...
  if (btnTabClient && btnTabTelemetry && viewTabClient && viewTabTelemetry) {
    ...
    const btnIssueTab = document.getElementById('tab-btn-run-issue');
    if (btnIssueTab) {
      btnIssueTab.addEventListener('click', issueAndAnchorTabCredential);
    }

    const btnCopyIssued = document.getElementById('btn-copy-issued-id');
    const tabIssueIdVal = document.getElementById('tab-issue-result-id');
    if (btnCopyIssued && tabIssueIdVal) {
      btnCopyIssued.addEventListener('click', () => {
        navigator.clipboard.writeText(tabIssueIdVal.textContent).then(() => {
          btnCopyIssued.textContent = 'Copied!';
          setTimeout(() => { btnCopyIssued.textContent = 'Copy'; }, 2000);
        });
      });
    }
  }
...
async function issueAndAnchorTabCredential() {
  const subjectInput = document.getElementById('issue-subject-input');
  const roleInput = document.getElementById('issue-role-input');
  const btnIssue = document.getElementById('tab-btn-run-issue');
  const resultPanel = document.getElementById('tab-issue-result');
  const resultIdVal = document.getElementById('tab-issue-result-id');

  if (!btnIssue || !resultPanel || !resultIdVal) return;

  const subject = subjectInput ? subjectInput.value.trim() : 'did:scatterid:alice';
  const role = roleInput ? roleInput.value.trim() : 'Lead Security Architect';

  btnIssue.disabled = true;
  btnIssue.textContent = 'Generating...';
  ...
  try {
    ...
    const data = await res.json();
    if (!res.ok) {
      alert('Failed to issue credential: ' + (data.error || 'Unknown error'));
      return;
    }

    resultPanel.classList.remove('hidden');
    resultIdVal.textContent = data.credentialId;
    ...
    if (typeof loadDatabaseExplorer === 'function') {
    ...
    btnIssue.disabled = false;
    btnIssue.textContent = 'Generate, Shard & Anchor Proof';
  }
}
  ...
  try {
    ...
    if (validCreds.length > 0) {
      ...
      validCreds.slice(0, 3).forEach(row => {
        ...
        pill.textContent = row.id.substring(0, 8) + '...' + row.id.slice(-4);
